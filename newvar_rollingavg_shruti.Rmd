...
```{r}
ListofHomeGamesByTeam_DateAndTeam <- HomeGamesByTeamList %>%
  lapply(function(x) x%>% select(GAME_DATE_EST, HOME_TEAM_ID)) %>% 
...
```{r}
ListofAwayGamesByTeam_DateAndTeam <- AwayGamesByTeamList %>%
  lapply(function(x) x%>% select(GAME_DATE_EST, VISITOR_TEAM_ID)) %>% 
  lapply(function(x) x%>% slice(1:5))
...
AvgPtsForRecent5GamesByTeam = merge(x = AvgPtsForRecent5HomeGamesByTeam, y = AvgPtsForRecent5AwayGamesByTeam, by = "TEAM_ID", all.x = TRUE)
 ...
 write.csv(AvgPtsForRecent5GamesByTeam,"last_5_avg_pts.csv", row.names = FALSE)
```
